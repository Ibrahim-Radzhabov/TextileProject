...
|---|--------|--------|-----------|--------|
| 1 | Увеличить вертикальные отступы секций | Очень высокий | Низкая | ⬜ |
| 2 | Next.js `<Image>` + blur placeholders | Очень высокий | Средняя | ⬜ |
| 3 | Увеличить gap продуктовой сетки | Высокий | Низкая | ⬜ |
| 4 | Убрать Cormorant Garamond | Высокий | Низкая | ⬜ |
| 5 | Увеличить body line-height | Средний | Низкая | ⬜ |
| 6 | Централизовать hardcoded цвета | Средний | Средняя | ⬜ |
| 7 | Проверить font-weight значения | Низкий | Низкая | ⬜ |
| 8 | Упростить neon-border поиска | Высокий | Средняя | ⬜ |
| 9 | Scroll-triggered reveals | Высокий | Средняя | ⬜ |
| 10 | Анимировать аккордеон chevron | Средний | Низкая | ⬜ |
| 11 | Уменьшить dock hover | Низкий | Низкая | ⬜ |
| 12 | Дифференцировать Button варианты | Средний | Низкая | ⬜ |
| 13 | Расширить PDP info panel | Средний | Низкая | ⬜ |
...
**Одно изменение с максимальным эффектом:** удвоить вертикальные отступы между секциями homepage. Это единственное CSS-изменение сдвинет восприятие с «хороший магазин» на «люксовый бренд».

**Три изменения для перехода 7.5 → 9.0:**
1. Отступы + воздух
2. Next.js Image с blur-up
3. Убрать neon-border, добавить scroll reveals

**Оставшийся путь 9.0 → 10.0** — это полировка деталей: типографика, микро-анимации, loading states, page transitions.

  ...
  return (
    ...
    >
      {items.map((item, index) => {
        ...
        const scale = prefersReducedMotion ? 1 : isHovered ? (isCapsule ? 1.04 : isMinimal ? 1.008 : 1.06) : 1;
        const y = prefersReducedMotion ? 0 : isHovered ? (isCapsule ? -1 : isMinimal ? -0.5 : -2) : 0;
          ...
          <>
            <span
              className={[
                ...
                isMinimal
                  ? "h-10 w-10 border border-transparent bg-transparent text-foreground/80 shadow-none hover:bg-foreground/[0.035] hover:text-foreground [&>svg]:h-5 [&>svg]:w-5 min-[1280px]:h-11 min-[1280px]:w-11"
                  :
                isCapsule
              ...
              {item.badge != null && item.badge > 0 && (
                <span
                  className={[
                    ...
                    isMinimal
                      ? "right-0 top-0 h-4 min-w-[16px] -translate-y-1/4 translate-x-[6px] border border-white/86 bg-[linear-gradient(180deg,#261f1a,#382920)] px-[3px] text-[10px] font-medium leading-[10px] tracking-[0.01em] text-background shadow-[0_8px_14px_-12px_rgba(0,0,0,0.5)]"
                      :
                    isCapsule
        ...
        const sharedProps = {
          ...
          className: [
            isMinimal
              ? "inline-flex rounded-full focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-foreground/20 focus-visible:ring-offset-4 focus-visible:ring-offset-background"

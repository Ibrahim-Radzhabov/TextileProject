...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  const response = await fetch(new URL("/catalog/products", resolveStoreApiUrl()), {
    method: "POST",
    headers: {
  ...
  if (!response.ok) {
    ...
    return NextResponse.redirect(
      addFlash(returnTo, "action_error", detail ?? "Не удалось создать товар."),
      303
    );
  }

  return NextResponse.redirect(addFlash(returnTo, "action_success", "Товар создан."), 303);
}
